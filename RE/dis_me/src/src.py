...
def encrypt(data,key):#加密主函数
    ...
    for i in range(4,36):
        x[i] = x[i - 4] ^ T(x[i - 3] ^ x[i - 2] ^ x[i - 1] ^ key[i])
...
def main():
    ...
    if len(flag) < 22 or len(flag) > 38:
        print ('wrong length!')
        return 
    match list(flag):
        case ['f', 'l', 'a', 'g', '{', *context, '}']:
            context = ''.join(context)
            context += '\x00' * (16 - len(context) % 16)
            str1 = context[0:16]
            str2 = context[16:32]
            iv = 0xc11f7376d80f02237e5af8f67f5852e7
            key = 0x5eeaf08fb6b140c70946186e582c620a
